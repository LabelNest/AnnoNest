...
const TaskCommand: React.FC<Props> = ({ tasks, userProfile }) => {
        ...
        <table className="w-full text-left border-collapse min-w-[1200px]">
          <thead>
            <tr className="bg-slate-950/40 border-b border-slate-800">
              ...
              <th className="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.4em]">Assignment</th>
              ...
              <tr key={task.id} className="hover:bg-slate-800/30 transition-all group cursor-pointer border-l-[6px] border-transparent hover:border-blue-500">
                <td className="px-10 py-8">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <p className="text-sm font-black text-white italic font-serif leading-none mb-1 uppercase tracking-tight">{task.id}</p>
                      <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">Project: {task.projectId}</p>
                    </div>
                  </div>
                </td>
                <td className="px-10 py-8">
                ...
                <td className="px-10 py-8">
                   <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-[10px] font-black text-slate-500 uppercase italic font-serif">
                        {task.assigned_to?.[0] || 'U'}
                      </div>
                      <span className="text-xs font-bold text-slate-400 italic">{task.assigned_to || 'Unassigned'}</span>
                   </div>
                </td>
                <td className="px-10 py-8">
            ...
            {filteredTasks.length === 0 && (
              <tr>
                <td colSpan={6} className="py-24 text-center">
                   ...
                   <p className="text-slate-600 font-black italic uppercase text-xs tracking-widest">No matching tasks found in current horizon.</p>
                </td>
              </tr>
            )}
          </tbody>
